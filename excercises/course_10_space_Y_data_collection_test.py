...
class Course10SpaceYDataCollectionTest:
    ...
    def get_rocket(self,id):
        url = Course10SpaceYDataCollectionTest._BASE_URL.format(Course10SpaceYDataCollectionTest._GET_ROCKET.format((id)))
        r = requests.get(url)
        return self.normalize_response(r)

    def get_launchpad(self,id):
        url = Course10SpaceYDataCollectionTest._BASE_URL.format(Course10SpaceYDataCollectionTest._GET_LAUNCHPAD.format((id)))
        r = requests.get(url)
        return self.normalize_response(r)

    def get_payload(self,id):
        url = Course10SpaceYDataCollectionTest._BASE_URL.format(
            Course10SpaceYDataCollectionTest._GET_PAYLOAD.format((id)))
        r = requests.get(url)
        return self.normalize_response(r)

    def get_launches(self):
        #url = Course10SpaceYDataCollectionTest._BASE_URL.format(Course10SpaceYDataCollectionTest._ALL_LAUNCHES)
        url=Course10SpaceYDataCollectionTest._GET_ALL_LAUNCHES #same as the prev
        r = requests.get(url)
        return self.normalize_response(r)

    def get_core(self,id):
        url = Course10SpaceYDataCollectionTest._BASE_URL.format(
            Course10SpaceYDataCollectionTest._GET_CORES.format((id)))
        r = requests.get(url)
        return self.normalize_response(r)
    ...
    def week_1_test(self):
        ...
        core_blocks,core_reuse_counts,core_serials,outcomes,flights,grid_fins,reused,legs,landing_pads=self.get_cores_data_col(launches_df)
